...
def genSine(A, f, phi, fs, t):
    ...
    return A * np.cos(2 * np.pi * f * T * np.arange(num_samples) + phi)

are_close = lambda a, b: all(np.isclose(x, y) for x, y in zip(a, b))

answers = [0.54030231, -0.63332387, -0.93171798,  0.05749049,  0.96724906]
prompt = [1, 10, 1, 50, .1]
assert are_close(genSine(*prompt), answers)
